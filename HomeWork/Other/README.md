...
>[!tip]
> ### Что делает этот код?
> Добавляет простой таймер, где по истечению времени пропадает(в зависимости от времени у таймера, меняется цикл дня и ночи)
...
`lighting` - **светильник для мира**

`lighting` нужен для:
1. Контроль над временем (день, ночь)
2. контроль света, тумана и других погодных условий
3. смена цвета эмбиента
4. 
### Аналогия:
...
`lighting` - Смена дня и ночи и так далее



>[!note]
> ### Что такое GetService вообще?
>**GetService** - Это вещь, где мы получаем библиотеку или с тем предметом, которым будем работать.
